...
import { PageHeader, Tabs, Select, Button, Badge, SearchInput, EmptyState } from "../ui/kit.jsx";
import DataTable from "../ui/DataTable.jsx";
import Icon from "../ui/icons.jsx";
import {
  STAGES, effectiveStage, stageTone, headlineQuote, quoteTotals, nextFollowUp,
  followUpState, paymentSummary, money, paxLabel, fmtDate, fmtDateTime,
} from "../lib/crm.js";
...
export default function Queries() {
  ...
  const columns = [
    {
      key: "guest", header: "Guest", render: (q) => (
    ...
    },
    {
      key: "trip", header: "Trip", render: (q) => (
        <div>
          ...
          <div className="tiny">{fmtDate(q.startDate)} · {q.nights || "?"}N · {paxLabel(q)}</div>
        </div>
      ),
    },
    {
  ...
  return (
    <div>
      ...
      <div className="row gap-3 mt-4" style={{ alignItems: "center", flexWrap: "wrap" }}>
        <SearchInput value={search} onChange={setSearch} placeholder="Search guest, phone, destination…" />
        {allTags.length > 0 && (
